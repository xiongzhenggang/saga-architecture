 ...
 * @className: EventuateKafkaMultiMessageConverter
 * @author: xzg
 * @description: TODO
 * @date: 22/11/2023-下午 10:27
 * @version: 1.0
 */
public class EventuateKafkaMultiMessageConverter {
    ...
    public static final byte[] MAGIC_ID_BYTES = StringBinaryMessageEncoding.stringToBytes(MAGIC_ID);
    ...
    private List<EventuateKafkaMultiMessagesHeader> decodeEventuateKafkaMultiMessagesHeaders(MultiMessageDecoder multiMessageDecoder) {
        ...
        for (int i = 0; i < headersDecoder.count(); i++) {
            ...
            String key = StringBinaryMessageEncoding.bytesToString(keyBytes);
            String value = StringBinaryMessageEncoding.bytesToString(valueBytes);
    ...
    public List<String> convertBytesToValues(byte[] bytes) {
        if (isMultiMessage(bytes)) {
            ...
            return Collections.singletonList(StringBinaryMessageEncoding.bytesToString(bytes));
        }
    }
